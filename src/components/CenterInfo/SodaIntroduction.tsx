...
function SodaIntroduction({ sodaData }: { sodaData: SodaInfoType }) {
  ...
  return (
    <div>
      <div className="mt-12 text-3xl text-blue-900 mxl:text-2xl mmd:mt-6">{sodaData.headText}</div>
      <div className="w-[100%] h-96 overflow-hidden mb-12 mmd:mb-6">
        <img src={sodaData.img} alt={sodaData.alt} className="mt-12 mmd:mt-6" />
      </div>
      <ul>
      ...
      <div className="flex justify-between gap-4 mb-12 mxl:flex-col mxl:items-center ">
        {sodaData.detailImg.map((imgAdress, index) => {
          return (
            <img
              src={imgAdress}
              alt="사무실 내부 사진"
